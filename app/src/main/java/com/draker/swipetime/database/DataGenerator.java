...
public class DataGenerator {
    ...
    public static void populateDatabase(Context context) {
        ...
        executor.execute(() -> {
            ...
            db.userDao().insert(getDefaultUser());
            db.achievementDao().insertAll(getAchievements());
    ...
    private static void clearDatabase(AppDatabase db) {
        ...
        db.userAchievementDao().deleteAll();
        db.achievementDao().deleteAll();
        db.movieDao().deleteAll();
        db.tvShowDao().deleteAll();
        db.gameDao().deleteAll();
        db.bookDao().deleteAll();
        db.animeDao().deleteAll();
    ...
    /**
     * Создать тестовый список достижений
     * @return список достижений
     */
    private static List<AchievementEntity> getAchievements() {
        List<AchievementEntity> achievements = new ArrayList<>();
        
        achievements.add(new AchievementEntity(
            "achievement-1",
            "Начинающий критик",
            "Оцените свой первый контент",
            "ic_achievement_beginner",
            50,
            "rate",
            1,
            "beginner"
        ...
        achievements.add(new AchievementEntity(
            "achievement-3",
            "Книжный червь",
            "Добавьте в избранное 5 книг",
            "ic_achievement_bookworm",
            200,
            "like_book",
            5,
            "collector"
        ...
        achievements.add(new AchievementEntity(
            "achievement-4",
            "Киноман",
            "Просмотрите 10 фильмов",
            "ic_achievement_movie",
            250,
            "view_movie",
            10,
            "collector"
